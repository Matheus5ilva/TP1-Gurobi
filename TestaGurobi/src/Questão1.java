...
public class Questão1 {
	...
	public static void main(String[] args) {
		...
		System.out.println("\tQuestão 1 - Problema do sapateiro");
		System.out.println();
		try {
		    ...
		    GRBVar x = model.addVar(0.0, 1.0, 0.0, GRB.BINARY, "x"); //Criação da variavel x = quantidade de sapatos feitos por hora, o metodo addVar(), o primeiro argumento usamos os limites inferiores (0.0) e o segundo argumento usamos para limites superiores (1.0). O terceiro argumento é o coeficiente objetivo linear(0.0). O quarto argumento é o tipo da variavel (GRB BINARY). O quinto argumento é o nome da variavel.   
		    GRBVar y = model.addVar(0.0, 1.0, 0.0, GRB.BINARY, "y"); //Criação de variavel y = quantidade de cintos feitos por hora, o metodo addVar(), o primeiro argumento usamos os limites inferiores (0.0) e o segundo argumento usamos para limites superiores (1.0). O terceiro argumento é o coeficiente objetivo linear(0.0). O quarto argumento é o tipo da variavel (GRB BINARY). O quinto argumento é o nome da variavel. 
		    
		    // A expressão de maximização é 5x + 2y
		    GRBLinExpr expr = new GRBLinExpr(); // Instanciamos um classe para função linear vazia para ser adicionar as variaveis.
		    expr.addTerm(5.0, x); //O metodo .addTerm() é para colocar o valor da constante da variavel x (sapato).
		    expr.addTerm(2.0, y); //O metodo .addTerm() é para colocar o valor da constante da variavel y (cinto).
		    ...
		    // A primeira restrição é do tempo 10x + 12y <= 60
		    expr = new GRBLinExpr(); // Instanciamos a classe para a função linear.
		    expr.addTerm(10.0, x); // O metodo .addTerm() é para colocarmos o valor da constante de x (sapato) das restrição.
		    expr.addTerm(12.0, y); // O metodo .addTerm() é para colocarmos o valor da constante de y (cinto) das restrição.
		    model.addConstr(expr, GRB.LESS_EQUAL, 60.0, "c0"); // O metodo .addConstr() para adicionarmos a restrição a um modelo espercifico. O primeiro argumento é o lado esquerdo da função (o que vem antes do operador de maior, menor ou igual). O segundo argumento é qual o sentido da restrição, aqui usamos o GRB_LESS_EQUAL (<=). O terceiro argumento é o lado direito da função.
		    
		    // A segunda restrição é do material (couro) 2x + y <= 6
		    expr = new GRBLinExpr(); // Instanciamos a classe para a função linear.
		    expr.addTerm(2.0, x); // O metodo .addTerm() é para colocarmos o valor da constante de x (sapato) das restrição.
		    expr.addTerm(1.0, y); // O metodo .addTerm() é para colocarmos o valor da constante de y (cinto) das restrição.
		    model.addConstr(expr, GRB.LESS_EQUAL, 6.0, "c1"); // O metodo .addConstr() para adicionarmos a restrição a um modelo espercifico. O primeiro argumento é o lado esquerdo da função (o que vem antes do operador de maior, menor ou igual). O segundo argumento é qual o sentido da restrição, aqui usamos o GRB_LESS_EQUAL (<=). O terceiro argumento é o lado direito da função.

...
public class Questao5 {
	...
	public static void main(String[] args) {
		...
		try {
			...
			env.set("logFile", "questao5.log");//Criamos um arquivo .log que sera usado para salvar os dados gerados pelo Gurobi
		    ...
		    // A expressão de maximização é 120x + 150y
		    GRBLinExpr expr = new GRBLinExpr(); // Instanciamos um classe para função linear vazia para ser adicionar as variaveis.
		    expr.addTerm(120.0, x); //O metodo .addTerm() é para colocar o valor da constante da variavel x (P1).
		    ...
		    // A primeira restrição é do recurso R1 2x + 4y <= 100
		    ...
		    expr.addTerm(4.0, y); // O metodo .addTerm() é para colocarmos o valor da constante de y (P2) das restrição.
		    model.addConstr(expr, GRB.LESS_EQUAL, 100.0, "c0"); // O metodo .addConstr() para adicionarmos a restrição a um modelo espercifico. O primeiro argumento é o lado esquerdo da função (o que vem antes do operador de maior, menor ou igual). O segundo argumento é qual o sentido da restrição, aqui usamos o GRB_LESS_EQUAL (<=). O terceiro argumento é o lado direito da função.
		    
		    // A segunda restrição é do recurso R2 3x + 2y <= 90
		    expr = new GRBLinExpr(); // Instanciamos a classe para a função linear.
		    expr.addTerm(3.0, x); // O metodo .addTerm() é para colocarmos o valor da constante de x (P1) das restrição.
		    expr.addTerm(2.0, y); // O metodo .addTerm() é para colocarmos o valor da constante de y (P2) das restrição.
		    model.addConstr(expr, GRB.LESS_EQUAL, 90.0, "c1"); // O metodo .addConstr() para adicionarmos a restrição a um modelo espercifico. O primeiro argumento é o lado esquerdo da função (o que vem antes do operador de maior, menor ou igual). O segundo argumento é qual o sentido da restrição, aqui usamos o GRB_LESS_EQUAL (<=). O terceiro argumento é o lado direito da função.
		    
		    // A terceira restrição é do recurso R3 5x + 3y <= 120
		    expr = new GRBLinExpr(); // Instanciamos a classe para a função linear.
		    expr.addTerm(5.0, x); // O metodo .addTerm() é para colocarmos o valor da constante de x (P1) das restrição.
		    expr.addTerm(3.0, y); // O metodo .addTerm() é para colocarmos o valor da constante de y (P2) das restrição.
		    model.addConstr(expr, GRB.LESS_EQUAL, 120.0, "c2"); // O metodo .addConstr() para adicionarmos a restrição a um modelo espercifico. O primeiro argumento é o lado esquerdo da função (o que vem antes do operador de maior, menor ou igual). O segundo argumento é qual o sentido da restrição, aqui usamos o GRB_LESS_EQUAL (<=). O terceiro argumento é o lado direito da função.

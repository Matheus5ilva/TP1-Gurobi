...
public class Questao3 {
	...
	public static void main(String[] args) {
		...
		try {
		    ...
		    GRBVar x = model.addVar(0.0, 1.0, 0.0, GRB.BINARY, "x"); //Criação da variavel x = frequencia semanal de A, o metodo addVar(), o primeiro argumento usamos os limites inferiores (0.0) e o segundo argumento usamos para limites superiores (1.0). O terceiro argumento é o coeficiente objetivo linear(0.0). O quarto argumento é o tipo da variavel (GRB BINARY). O quinto argumento é o nome da variavel.   
		    GRBVar y = model.addVar(0.0, 1.0, 0.0, GRB.BINARY, "y"); //Criação de variavel y = frequencia semanal de B, o metodo addVar(), o primeiro argumento usamos os limites inferiores (0.0) e o segundo argumento usamos para limites superiores (1.0). O terceiro argumento é o coeficiente objetivo linear(0.0). O quarto argumento é o tipo da variavel (GRB BINARY). O quinto argumento é o nome da variavel.  
		    
		    // A expressão de maximização é 30000x + 10000y
		    GRBLinExpr expr = new GRBLinExpr(); // Instanciamos um classe para função linear vazia para ser adicionar as variaveis.
		    expr.addTerm(30000.0, x); //O metodo .addTerm() é para colocar o valor da constante da variavel x (A).
		    expr.addTerm(10000.0, y); //O metodo .addTerm() é para colocar o valor da constante da variavel y (B).
		    ...
		    // A primeira restrição é do musical 20x + 10y <= 80
		    expr = new GRBLinExpr(); // Instanciamos a classe para a função linear.
		    expr.addTerm(20.0, x); // O metodo .addTerm() é para colocarmos o valor da constante de x (A) das restrição.
		    expr.addTerm(10.0, y); // O metodo .addTerm() é para colocarmos o valor da constante de y (B) das restrição.
		    model.addConstr(expr, GRB.LESS_EQUAL, 80.0, "c0"); // O metodo .addConstr() para adicionarmos a restrição a um modelo espercifico. O primeiro argumento é o lado esquerdo da função (o que vem antes do operador de maior, menor ou igual). O segundo argumento é qual o sentido da restrição, aqui usamos o GRB_LESS_EQUAL (<=). O terceiro argumento é o lado direito da função.
		    
		    // A segunda restrição é da propaganda x + y >= 5
		    expr = new GRBLinExpr(); // Instanciamos a classe para a função linear.
		    expr.addTerm(1.0, x); // O metodo .addTerm() é para colocarmos o valor da constante de x (A) das restrição.
		    expr.addTerm(1.0, y); // O metodo .addTerm() é para colocarmos o valor da constante de y (B) das restrição.
		    model.addConstr(expr, GRB.GREATER_EQUAL, 5.0, "c1"); // O metodo .addConstr() para adicionarmos a restrição a um modelo espercifico. O primeiro argumento é o lado esquerdo da função (o que vem antes do operador de maior, menor ou igual). O segundo argumento é qual o sentido da restrição, aqui usamos o GRB_GREATER_EQUAL (>=). O terceiro argumento é o lado direito da função.
